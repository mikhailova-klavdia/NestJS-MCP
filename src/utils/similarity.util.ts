...
export function jaccardSimilarity(vecA: number[], vecB: number[]): number {
  validateVectors(vecA, vecB);
  const intersection = vecA.reduce(
    (sum, a, idx) => sum + (a && vecB[idx] ? 1 : 0),
    0
  );
...
function validateVectors(vecA: number[], vecB: number[]): void {
